...
When this flowchart is executed, the following happens:
...
After executing the flowchart, the output model generated by each node is saved. Selecting the node will display the model in the viewer on the right. If more than one node is selected, then the model in teh last selected node will be displayed.
...
* The parameters for the script are defined in the `Start` node.
...
* When importing a Mobius script as a _global functions_, the return value of the global function can be specified in the `Return` input box in the `End` node. This is discussed in more detail in the section on [Global Functions].
